...
function getFile(file: Buffer | string, isPath: boolean = true) {
	return new Promise<Buffer | string>((resolve, reject) => {
		if (!isPath) return resolve(file); // From zip;
...
export default function csvToJson<DataType = any>(
	...
	options: {
		...
		headers?: string[];
		isPath?: boolean; // Check if the file comes from a path or from a zip
	...
	return new Promise<DataType[]>(async (resolve, reject) => {
		try {
			...
			const data = await getFile(file, options.isPath);
