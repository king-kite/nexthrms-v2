...
export const projectCreateSchema = object({
  ...
  team: array()
    .of(
      object({
        // id: Joi.string().optional().allow('').label('Team Member ID'),
        employeeId: string().required().label('Employee ID'),
        isLeader: boolean().optional().default(false).label('Is Team Leader'),
      })
    )
    .optional()
    .label('Team'),
  client: string().nullable().optional().label('Client ID'),
});
...
export const taskCreateSchema = object({
  ...
  followers: array()
    .of(
      object({
        // id: Joi.string().optional().allow('').label('Team Member ID'),
        memberId: string().required().label('Team Member ID'),
