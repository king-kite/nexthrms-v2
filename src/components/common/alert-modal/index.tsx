...
const wrapperStyle =
	'duration-500 fixed flex h-full items-center left-0 main-container-width top-0 transform transition-opacity z-[200] lg:right-0';
...
const AlertModal = ({
	...
	return (
		...
		>
			...
			>
				<div
					className={`${bgColor} flex items-center justify-between px-4 py-2 rounded-t-lg`}
				>
					<h6
						className={`${
				...
				<div
					className={`bg-white flex flex-col items-center relative px-5 py-4 ${
						footer ? '' : 'rounded-b-lg'
					}`}
				>
					{Icon && (
						<span
							className={
								iconColor +
								' border flex items-center justify-center rounded-round mt-2 p-4 text-xs'
							}
						>
							<Icon
								className={
									iconColor + ' text-lg sm:text-xl md:text-2xl lg:text-3xl'
								}
							/>
						</span>
					)}
					...
					{decisions && (
						<div
							className="flex flex-wrap mt-3 items-center justify-around w-full"
							style={{ maxWidth: '12rem' }}
						>
							{decisions.map((decision, index) => (
								<div
									key={index}
									onClick={closeOnButtonClick ? close : undefined}
								>
					...
					{loading && (
						<div
							className={`${
								footer ? '' : 'rounded-b-lg'
							} absolute bg-dark-transparent left-0 h-full top-0 w-full`}
						>
							<div className="flex justify-center h-[85%] w-full">
								<Loader width="md" color="white" size={7} type="dotted" />
							</div>
						</div>
					)}
				</div>
