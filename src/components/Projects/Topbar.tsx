...
import { fileRequirements, followerRequirements, projectRequirements, taskRequirements, teamRequirements } from './requirements';
...
const Topbar = ({ loading, openModal, onSubmit, exportData }: TopbarProps) => {
	...
	const [formType, setFormType] = React.useState<'projects' | 'files' | 'team' | 'tasks' | 'followers'>('projects');
	const [modalVisible, setModalVisible] = React.useState(false);

	const { open } = useAlertContext();
	const { data: authData } = useAuthContext();

	const { description, requirements, sampleUrl, title, url } = React.useMemo(() => {
		...
		return {
			...
			description:
				'Upload a valid file to import multiple projects.',			
		}
	}, [formType]);

	const [canCreate, canCreateTask, canCreateProjectFile, canEditProject, canEditTask] = React.useMemo(() => {
		...
		const canCreateTask = authData
			? authData.isSuperUser ||
			  authData.isAdmin && hasModelPermission(authData.permissions, [permissions.projecttask.CREATE])
			: false;
		const canCreateProjectFile = authData
			? authData.isSuperUser ||
			  authData.isAdmin && hasModelPermission(authData.permissions, [permissions.projectfile.CREATE])
			: false;
		const canEditProject = authData
			? authData.isSuperUser ||
			  authData?.isAdmin && hasModelPermission(authData.permissions, [permissions.project.EDIT])
			: false;
		const canEditTask = authData
			? authData.isSuperUser ||
			  authData.isAdmin && hasModelPermission(authData.permissions, [permissions.projecttask.EDIT])
			: false;
		return [canCreateProject, canCreateTask, canCreateProjectFile, canEditProject, canEditTask];
	}, [authData]);
	...
	const actionList = React.useMemo(() => {
		...
		return list
	}, [canCreateProject, canCreateTask, canCreateProjectFile, canEditProject, canEditTask])
