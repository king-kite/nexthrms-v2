...
const usePagination = ({
	...
	const totalPages = React.useMemo(() => Math.ceil(totalItems / pageSize), [totalItems, pageSize]);
	const [currentPage, setCurrentPage] = React.useState(current);
	const [endPage, setEndPage] = React.useState(maxPages > totalPages ? totalPages : maxPages);
